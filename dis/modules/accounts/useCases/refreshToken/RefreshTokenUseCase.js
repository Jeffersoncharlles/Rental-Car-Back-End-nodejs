...
let RefreshTokenUseCase = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute(token) {
    ...
    const {
      email,
      sub
    } = (0, _jsonwebtoken.verify)(token, _auth.default.secret_refresh_token); // pega o id do usuario do token

    const user_id = sub; // busca o token e verifica o usuario

    const userToken = await this.tokensRepository.findByUserIdAndToken(user_id, token); // se o token nao for o mesmo manda msg
    ...
    await this.tokensRepository.deleteByID(userToken.id); // criar novo refresh_token
    ...
    const expires_date = this.dateProvider.addDays(_auth.default.expires_refresh_token_days); // salvar no db
